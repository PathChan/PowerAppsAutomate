...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	...
	if logging.getLogger().hasHandlers() and not force_setup:
		return logging.getLogger('mocProcessing')
	...
	class BrowserUseFormatter(logging.Formatter):
		...
		def format(self, record):
			...
			if self.log_level > logging.DEBUG and isinstance(record.name, str) and record.name.startswith('mocProcessing.'):
				...
				if 'Agent' in record.name:
					...
					record.name = 'dom'
				elif record.name.startswith('mocProcessing.'):
					# For other mocProcessing modules, use the last part
	...
	# Configure mocProcessing logger
	browser_use_logger = logging.getLogger('mocProcessing')
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
	...
	for name in ['mocProcessing.agent', 'mocProcessing.tools']:
	...
	for name in ['bubus', 'mocProcessing.browser.session']:
		logger = logging.getLogger(name)
