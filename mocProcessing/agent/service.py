...
if TYPE_CHECKING:
	pass  # Skill stubs removed in mocProcessing

from dotenv import load_dotenv

from mocProcessing.agent.cloud_events import (
...
from mocProcessing.agent.message_manager.utils import save_conversation
from mocProcessing.llm.base import BaseChatModel
from mocProcessing.llm.exceptions import ModelProviderError, ModelRateLimitError
from mocProcessing.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from mocProcessing.tokens.service import TokenCost
...
from mocProcessing import Browser, BrowserProfile, BrowserSession
from mocProcessing.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from mocProcessing.agent.gif import create_history_gif
from mocProcessing.agent.message_manager.service import (
	MessageManager,
)
from mocProcessing.agent.prompts import SystemPrompt
from mocProcessing.agent.views import (
	ActionResult,
	AgentError,
	AgentHistory,
	AgentHistoryList,
...
from mocProcessing.browser.events import _get_timeout
from mocProcessing.browser.session import DEFAULT_BROWSER_PROFILE
from mocProcessing.browser.views import BrowserStateSummary
from mocProcessing.config import CONFIG
from mocProcessing.dom.views import DOMInteractedElement, MatchLevel
from mocProcessing.filesystem.file_system import FileSystem
from mocProcessing.observability import observe, observe_debug
from mocProcessing.telemetry.service import ProductTelemetry
from mocProcessing.telemetry.views import AgentTelemetryEvent
from mocProcessing.tools.registry.views import ActionModel
from mocProcessing.tools.service import Tools
from mocProcessing.utils import (
	URL_PATTERN,
	_log_pretty_path,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from mocProcessing.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from mocProcessing import ChatBrowserUse
		...
		# Skills integration removed in mocProcessing
		self.skill_service = skill_service
		self._skills_registered = False
		if skill_ids:
			logger.warning('skill_ids parameter ignored — skills support removed in mocProcessing')
		...
		# ChatAnthropic removed in mocProcessing — always False
		is_anthropic = False
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'mocProcessing.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from mocProcessing.screenshots.service import ScreenshotService
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from mocProcessing.llm.messages import AssistantMessage, UserMessage
	...
	def _log_agent_event(self, max_steps: int, agent_run_error: str | None = None) -> None:
		...
		# 容错：不同版本 TokenCost 的 API 不一致（可能缺少 get_usage_tokens_for_model）
		class _EmptyTokenSummary:
			prompt_tokens = 0
			completion_tokens = 0
		...
		try:
			...
			self.logger.debug(f'token_cost_service.get_usage_tokens_for_model failed: {_tc_err}')
			token_summary = _EmptyTokenSummary()
	...
	async def run(
		...
		from mocProcessing.utils import SignalHandler
		...
		try:
			...
			# Log token usage summary（容错：TokenCost 在某些版本中 log_usage_summary 不是 coroutine）
			try:
				result = self.token_cost_service.log_usage_summary()
				if asyncio.iscoroutine(result):
					await result
			except Exception as _tc_err:
				self.logger.debug(f'token_cost_service.log_usage_summary failed: {_tc_err}')
			...
			# Generate GIF if needed - DISABLED in mocProcessing (gif module removed)
			if self.settings.generate_gif:
				self.logger.warning('generate_gif=True ignored — GIF support removed in mocProcessing')
	...
	async def _generate_rerun_summary(
		...
		from mocProcessing.agent.views import RerunSummaryAction
		...
		from mocProcessing.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			from mocProcessing.llm.messages import BaseMessage
	...
	async def _execute_ai_step(
		...
		"""
		from mocProcessing.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from mocProcessing.llm.messages import SystemMessage, UserMessage
		from mocProcessing.utils import sanitize_surrogates
		...
		try:
			from mocProcessing.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from mocProcessing.agent.variable_detector import detect_variables_in_history
