...
from mocProcessing.browser.views import PLACEHOLDER_4PX_SCREENSHOT
from mocProcessing.dom.views import NodeType, SimplifiedNode
from mocProcessing.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from mocProcessing.observability import observe_debug
from mocProcessing.utils import is_new_tab_page, sanitize_surrogates

if TYPE_CHECKING:
	from mocProcessing.agent.views import AgentStepInfo
...
class SystemPrompt:
	...
	def _load_prompt_template(self) -> None:
		"""Load the unified system prompt and experience template from markdown files."""
		try:
			# Always use the main system_prompt.md (no model-specific branching)
			template_filename = 'system_prompt.md'

			with (
				importlib.resources.files('mocProcessing.agent.system_prompts')
				.joinpath(template_filename)
				.open('r', encoding='utf-8') as f
			):
				self.prompt_template = f.read()

			# Append experience template if it exists
			experience_filename = 'experience.md'
			try:
				with (
					importlib.resources.files('mocProcessing.agent.system_prompts')
					.joinpath(experience_filename)
					.open('r', encoding='utf-8') as f
				):
					experience_content = f.read()
					if experience_content.strip():
						# Escape curly braces so JavaScript examples in experience.md
						# don't break Python str.format() (e.g. { try {, { if (guard) {)
						experience_content = experience_content.replace('{', '{{').replace('}', '}}')
						self.prompt_template += '\n\n' + experience_content
			except FileNotFoundError:
				pass  # experience.md is optional
