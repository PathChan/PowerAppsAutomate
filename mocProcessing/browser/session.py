...
from mocProcessing.browser._cdp_timeout import TimeoutWrappedCDPClient
from mocProcessing.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError

# CDP logging is now handled by setup_logging() in logging_config.py
# It automatically sets CDP logs to the same level as mocProcessing logs
from mocProcessing.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
...
_UNSET: Any = object()
from mocProcessing.browser.events import (
...
from mocProcessing.browser.profile import BrowserProfile, ProxySettings
from mocProcessing.browser.views import BrowserStateSummary, TabInfo
from mocProcessing.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from mocProcessing.observability import observe_debug
from mocProcessing.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from mocProcessing.actor.page import Page
	from mocProcessing.browser.demo_mode import DemoMode
	from mocProcessing.browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		"""[NOT SUPPORTED in mocProcessing] Use BrowserSession() with explicit executable_path/user_data_dir."""
		raise NotImplementedError(
			'from_system_chrome() removed in mocProcessing. '
			'Pass executable_path and user_data_dir to BrowserSession() directly.'
		)

	@classmethod
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		"""[NOT SUPPORTED in mocProcessing]"""
		raise NotImplementedError('list_chrome_profiles() removed in mocProcessing.')
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'mocProcessing.{self}')
		return logging.getLogger(f'mocProcessing.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from mocProcessing.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from mocProcessing.browser.events import SaveStorageStateEvent
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from mocProcessing.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from mocProcessing.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from mocProcessing.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from mocProcessing.browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from mocProcessing.browser.crash_watchdog import CrashWatchdog
		from mocProcessing.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from mocProcessing.browser.watchdogs.dom_watchdog import DOMWatchdog
		from mocProcessing.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from mocProcessing.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from mocProcessing.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from mocProcessing.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from mocProcessing.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from mocProcessing.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from mocProcessing.browser.watchdogs.security_watchdog import SecurityWatchdog
		from mocProcessing.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
		...
		# RecordingWatchdog disabled: video recording not needed and module is missing.
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		if not self.cdp_url.startswith('ws'):
			...
			async with httpx.AsyncClient(timeout=httpx.Timeout(30.0), trust_env=not is_localhost) as client:
				...
				from mocProcessing.utils import get_browser_use_version
		...
		try:
			...
			from mocProcessing.browser.session_manager import SessionManager
			...
			from mocProcessing.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from mocProcessing.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from mocProcessing.dom.views import NodeType
	...
	def find_file_input_near_element(
		...
		"""
		from mocProcessing.dom.views import EnhancedDOMTreeNode
