...
from mocProcessing.actor.utils import get_key_info
...
from mocProcessing.browser.views import BrowserError, URLNotAllowedError
from mocProcessing.browser.watchdog_base import BaseWatchdog
from mocProcessing.dom.service import EnhancedDOMTreeNode
...
class DefaultActionWatchdog(BaseWatchdog):
	...
	async def _handle_print_button_click(self, element_node: EnhancedDOMTreeNode) -> dict | None:
		...
		try:
			...
			from mocProcessing.browser.events import FileDownloadedEvent
	...
	async def _click_element_node_impl(self, element_node) -> dict | None:
		...
		try:
			...
			try:
				...
				# [PowerApps] After any click, try to focus formula bar via button click
				await self._ensure_powerapps_formula_focused(cdp_session, session_id)
	...
	# ── PowerApps formula bar focus ────────────────────────────────────────
	# PowerApps 公式栏使用 Monaco Editor，但 CDP 的 mouse event 无法正确触发焦点转移。
	# 唯一可靠的方法是点击 `#formulaBarContainer > button`，该按钮的 click handler
	# 会触发 Monaco 内部的焦点流程，把光标放到隐藏 textarea 的最后。
	# 其他任何方法（Monaco API、textarea.focus()、overflow-guard click）都已验证无效。

	_POWERAPPS_FOCUS_JS = textwrap.dedent("""\
		(function() {
			var fb = document.querySelector('#formulaBarContainer > button');
			if (fb) { fb.click(); return 'OK'; }
			return 'NO_BUTTON';
		})()
	""")

	async def _ensure_powerapps_formula_focused(self, cdp_session, session_id: str) -> bool:
		"""After any click on a PowerApps page, click the formula bar button to focus Monaco.

		This is the ONLY reliably working method for PowerApps formula bar focus.
		CDP mouse events / Monaco API / textarea.focus() all fail in PowerApps.
		"""
		try:
			r = await cdp_session.cdp_client.send.Runtime.evaluate(
				params={
					'expression': '!!document.querySelector("#formulaBarContainer > button")',
					'returnByValue': True,
				},
				session_id=session_id,
			)
			has_button = r.get('result', {}).get('value', False)
		except Exception:
			return False
		...
		self.logger.debug('🎯 PowerApps formula bar detected, clicking formula bar button to focus Monaco...')
		try:
			result = await cdp_session.cdp_client.send.Runtime.evaluate(
				params={'expression': self._POWERAPPS_FOCUS_JS, 'returnByValue': True},
				session_id=session_id,
			)
			status = result.get('result', {}).get('value', 'FAIL')
			self.logger.debug(f'Formula bar focus result: {status}')
			return status == 'OK'
		except Exception as e:
			self.logger.debug(f'Formula bar focus failed: {e}')
			return False
