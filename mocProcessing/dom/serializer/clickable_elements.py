from mocProcessing.dom.views import EnhancedDOMTreeNode, NodeType
...
class ClickableElementDetector:
	...
	def is_interactive(node: EnhancedDOMTreeNode) -> bool:
		...
		# PowerApps formula editor uses Monaco. The .overflow-guard div is the
		# visible click layer that receives all mouse events. The ghost <textarea>
		# inside it (1x1, opacity:0) is not detected by normal heuristics.
		# We catch Monaco editors here by class so they get an [index] even when
		# JS listener detection is skipped (e.g. pages with >10k elements).
		if node.attributes:
			cls = node.attributes.get('class', '')
			# 1) The .overflow-guard layer: Monaco's click target
			if 'overflow-guard' in cls:
				return True
			# 2) The .monaco-editor container itself
			if 'monaco-editor' in cls:
				return True
