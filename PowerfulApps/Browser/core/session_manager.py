...
if TYPE_CHECKING:
	from PowerfulApps.Browser.core.session import BrowserSession, CDPSession, Target
...
class SessionManager:
	...
	async def _handle_target_detached(self, event: DetachedFromTargetEvent) -> None:
		...
		if target_fully_removed:
			if target_type in ('page', 'tab'):
				from PowerfulApps.Browser.core.events import TabClosedEvent
	...
	async def _recover_agent_focus(self, crashed_target_id: TargetID) -> None:
		...
		try:
			...
			if page_targets:
				...
				from PowerfulApps.Browser.core.events import TabCreatedEvent
			...
			if new_session:
				...
				from PowerfulApps.Browser.core.events import AgentFocusChangedEvent
			...
			for _ in range(20):
				...
				if fallback_session:
					...
					from PowerfulApps.Browser.core.events import AgentFocusChangedEvent, TabCreatedEvent
