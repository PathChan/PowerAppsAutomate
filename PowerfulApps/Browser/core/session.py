...
from PowerfulApps.Browser.cdp._cdp_timeout import TimeoutWrappedCDPClient
from PowerfulApps.Browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError

# CDP logging is now handled by setup_logging() in logging_config.py
# It automatically sets CDP logs to the same level as PowerfulApps logs
from PowerfulApps.Browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
...
from PowerfulApps.DOM.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from PowerfulApps.Browser.utils.moc_observability import observe_debug
from PowerfulApps.Browser.utils.moc_utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from PowerfulApps.Browser.actor.page import Page
	from PowerfulApps.Browser.demo.demo_mode import DemoMode
	from PowerfulApps.Browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		"""[NOT SUPPORTED in PowerfulApps] Use BrowserSession() with explicit executable_path/user_data_dir."""
		raise NotImplementedError(
			'from_system_chrome() removed in PowerfulApps. '
			'Pass executable_path and user_data_dir to BrowserSession() directly.'
		)

	@classmethod
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		"""[NOT SUPPORTED in PowerfulApps]"""
		raise NotImplementedError('list_chrome_profiles() removed in PowerfulApps.')
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'PowerfulApps.{self}')
		return logging.getLogger(f'PowerfulApps.{self}')
	...
	async def kill(self) -> None:
		...
		from PowerfulApps.Browser.core.events import SaveStorageStateEvent
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from PowerfulApps.Browser.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from PowerfulApps.Browser.actor.page import Page as PageActor
	...
	async def close_page(self, page: 'Union[Page, str]') -> None:
		...
		# Import here to avoid circular import
		from PowerfulApps.Browser.moc_actor.page import Page as Target
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from PowerfulApps.Browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from PowerfulApps.Browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from PowerfulApps.Browser.crash_watchdog import CrashWatchdog
		from PowerfulApps.Browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from PowerfulApps.Browser.watchdogs.dom_watchdog import DOMWatchdog
		from PowerfulApps.Browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from PowerfulApps.Browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from PowerfulApps.Browser.watchdogs.popups_watchdog import PopupsWatchdog
		from PowerfulApps.Browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from PowerfulApps.Browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		if not self.cdp_url.startswith('ws'):
			...
			async with httpx.AsyncClient(timeout=httpx.Timeout(30.0), trust_env=not is_localhost) as client:
				...
				from PowerfulApps.Browser.utils.moc_utils import get_browser_use_version
		...
		try:
			...
			from PowerfulApps.Browser.core.session_manager import SessionManager
			...
			from PowerfulApps.Browser.utils.moc_utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from PowerfulApps.Browser.core.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from PowerfulApps.DOM.views import NodeType
	...
	def find_file_input_near_element(
		...
		"""
		from PowerfulApps.DOM.views import EnhancedDOMTreeNode
