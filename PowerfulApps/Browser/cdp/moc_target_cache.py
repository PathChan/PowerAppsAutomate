...
def _read_cache() -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("dom_targets.json 损坏，已重新开始：%s", e)
        return {}
...
async def _locate_and_record(
    ...
    if not info or not info.get("found"):
        logger.info("定位器未命中：目标键=%s 标签=%s 详情=%s", key, label, info)
        return None
    ...
    logger.info("已缓存 PowerApps 目标 %s @ (%.1f, %.1f)", key, cx, cy)
    return entry
...
async def click_via_cache(
    ...
    *,
    ...
    if entry and not _viewport_matches(entry, vw, vh, dpr):
        logger.info("视口已变化，刷新目标缓存：%s", key)
        entry = None
    ...
    while attempts <= max_retries:
        ...
        if not entry:
            ...
            if not entry:
                return {"ok": False, "error": f"定位器执行失败：{key}"}

        x = entry["x"]
        y = entry["y"]
        if used_source == "cache":
            logger.info("使用缓存的 PowerApps 目标 %s @ (%.1f, %.1f)", key, x, y)
        else:
            logger.info("使用重新定位的 PowerApps 目标 %s @ (%.1f, %.1f)", key, x, y)
        await _click_at(x, y)
        ...
        try:
            ...
            logger.debug("校验目标时抛出异常：%s，异常=%s", key, e)

        if ok:
            return {"ok": True, "source": used_source, "x": x, "y": y, "key": key}

        # 失败：作废缓存重试
        logger.info("缓存点击失败，作废并重试：%s", key)
        invalidate(key)
        entry = None

    return {"ok": False, "error": f"缓存点击重试次数已耗尽：{key}"}
